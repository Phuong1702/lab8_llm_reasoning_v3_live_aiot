...
camera_cap = None
camera_yolo_model = None
latest_frame_jpeg: Optional[bytes] = None

latest_camera_event: Dict[str, Any] = {
    'camera_enabled': False,
    'motion_detected': False,
    'activity_level': 'NONE',
    'person_count': 0,
    'confidence': 0.0,
    'event_type': 'NO_CAMERA',
    'source': 'simulator',
    'detector': 'none',
    'detections': [],
    'timestamp': None,
...
def _load_yolo_model(model_name: str = 'yolov8n.pt'):
    global camera_yolo_model
    if YOLO is None:
        raise RuntimeError('ultralytics is not installed. Run: pip install ultralytics')
    if camera_yolo_model is None:
        camera_yolo_model = YOLO(model_name)
    return camera_yolo_model


def _camera_loop(camera_index: int = 0, use_yolo: bool = True, yolo_model: str = 'yolov8n.pt', conf_threshold: float = 0.35) -> None:
    global camera_running, camera_cap, latest_camera_event, latest_frame_jpeg
    ...
    if cv2 is None:
        with camera_lock:
            latest_camera_event = {
                ...
                'person_count': 0,
                'confidence': 0.0,
                'event_type': 'OPENCV_NOT_INSTALLED',
                'source': 'webcam',
                'detector': 'none',
                'detections': [],
                'timestamp': now_iso(),
    ...
    if not cap.isOpened():
        with camera_lock:
            latest_camera_event = {
                ...
                'detections': [],
                'timestamp': now_iso(),
                'error': f'Cannot open camera_index={camera_index}. Try camera_index=1.',
            }
        camera_running = False
        return
    ...
    if use_yolo:
        try:
            detector = _load_yolo_model(yolo_model)
            detector_name = f'yolo:{yolo_model}'
        except Exception as e:
            detector = None
            detector_name = 'motion-fallback'
            yolo_error = str(e)
    ...
    while camera_running:
        ...
        # Motion evidence from Lab 6 style
        ...
        # YOLO person detection from Lab 7 style
        ...
        if detector is None:
            person_count = 1 if motion_detected else 0
            confidence = 0.65 if motion_detected else 0.50

        if person_count >= 30:
            event_type = 'CROWDED_ROOM'
        elif person_count > 0:
            event_type = 'ROOM_OCCUPIED'
        else:
            event_type = 'EMPTY_ROOM'

        cv2.putText(frame, f'people={person_count} detector={detector_name}', (12, 28),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

        ok, jpeg = cv2.imencode('.jpg', frame)
        with camera_lock:
            latest_frame_jpeg = jpeg.tobytes() if ok else None
            latest_camera_event = {
                'camera_enabled': True,
    ...
    cap.release()
    with camera_lock:
        latest_camera_event['camera_enabled'] = False


def get_camera_event_snapshot() -> Dict[str, Any]:
    with camera_lock:
        return dict(latest_camera_event)
...
def previous_ai_model_outputs(sensors: Dict[str, Any], scenario_id: str) -> Dict[str, Any]:
    ...
    if scenario_id == 'lab_overcrowded_high_co2':
        ...
        cam = get_camera_event_snapshot()
        use_cam = bool(cam.get('camera_enabled'))
        if use_cam:
            people = int(cam.get('person_count', 0))
            vision_confidence = cam.get('confidence', sensors.get('vision_confidence'))
            motion_detected = bool(cam.get('motion_detected'))
            activity_level = cam.get('activity_level', 'LOW')
            event_type = cam.get('event_type', 'EMPTY_ROOM')
            source = 'webcam'
            detector = cam.get('detector', 'webcam')
            detections = cam.get('detections', [])
        else:
            vision_confidence = sensors.get('vision_confidence')
            motion_detected = people > 0
            activity_level = 'HIGH' if people > 30 else 'MEDIUM' if people > 10 else 'LOW'
            event_type = 'CROWDED_ROOM' if people >= 30 else 'ROOM_OCCUPIED' if people > 0 else 'EMPTY_ROOM'
            source = 'simulator'
            detector = 'simulator'
            detections = []
        ...
        return {
            ...
            'lab6_motion_event': {'motion_detected': motion_detected, 'activity_level': activity_level, 'source': source},
            'lab7_vision_event': {'person_count': people, 'confidence': vision_confidence, 'event_type': event_type, 'source': source, 'detector': detector, 'detections': detections}
        }
...
def _extract_json_object(raw: str) -> str:
    ...
    This makes local LLM output more tolerant if it adds extra text.
    """
    raw = (raw or '').strip()
...
def call_ollama(context: Dict[str, Any], model: Optional[str] = None) -> Dict[str, Any]:
    ...
    # qwen3:0.6b on Ollama exposes completion/generate reliably.
    # Some installations return 404 for /api/chat, so Lab 8 uses /api/generate.
    r = requests.post(
        f'{base}/api/generate',
        json=payload,
...
def call_gemini(context: Dict[str, Any], model: Optional[str] = None) -> Dict[str, Any]:
    """Call Gemini API for Lab 8 API mode.
    Requires:
    ...
    prompt = (
        ...
        'Chỉ sử dụng context được cung cấp.\n'
        'Không bịa thêm giá trị cảm biến hoặc kết quả model.\n'
        'Trả lời bằng tiếng Việt rõ ràng, dễ hiểu cho người vận hành.\n'
        'Không cho phép điều khiển tự động trong lab mode.\n'
        'control_allowed luôn phải là false.\n'
        'Nếu evidence mâu thuẫn, confidence thấp hoặc có rủi ro vận hành, need_human_review=true.\n'
        'Trả về DUY NHẤT một JSON object, không markdown, không thêm chữ ngoài JSON.\n\n'
        'BẮT BUỘC trả đúng đủ 7 key sau:\n'
        '{\n'
        '  "situation_summary": "Tóm tắt tình huống bằng tiếng Việt",\n'
        '  "risk_level": "LOW hoặc MEDIUM hoặc HIGH hoặc CRITICAL",\n'
        '  "recommended_action": "Hành động khuyến nghị bằng tiếng Việt",\n'
        '  "control_allowed": false,\n'
        '  "need_human_review": true,\n'
        '  "blocked_reason": "Lý do không cho điều khiển tự động hoặc lý do cần human review",\n'
        '  "evidence_used": ["telemetry", "lab3_anomaly_detection", "lab4_forecasting", "lab6_camera_motion_metadata", "lab7_vision_ai", "safety_rules"]\n'
        '}\n\n'
        + build_prompt(context)
    )
...
def camera_start(camera_index: int = 0, use_yolo: bool = True, yolo_model: str = 'yolov8n.pt', conf_threshold: float = 0.35):
    """Start webcam capture. If use_yolo=true, detect class person using YOLO."""
    global camera_running, camera_thread
    if camera_running:
        return {'status': 'already_running', 'camera_event': get_camera_event_snapshot()}

    camera_running = True
    camera_thread = threading.Thread(
        target=_camera_loop,
        args=(camera_index, use_yolo, yolo_model, conf_threshold),
        daemon=True,
    )
    camera_thread.start()
    return {
        ...
        'note': 'Open /camera/latest to see person_count. Open /camera/frame to see latest annotated frame.'
    }
...
def camera_latest():
    return get_camera_event_snapshot()
...
def camera_frame():
    ...
    if frame is None:
        raise HTTPException(status_code=404, detail='No camera frame available. Start camera first.')
    return StreamingResponse(iter([frame]), media_type='image/jpeg')



@app.get('/demo-summary/{scenario_id}')
def demo_summary(scenario_id: str, mode: str = 'local'):
    if scenario_id not in SCENARIOS:
        raise HTTPException(status_code=404, detail='Unknown scenario')

    result = compare_three_levels(scenario_id, mode=mode)
    telemetry = result['telemetry']
    sensor_only = result['sensor_only']
    ai_models = result['sensor_plus_ai_models']
    llm = result['sensor_ai_llm']
    ai_outputs = ai_models.get('ai_model_outputs', {})
    vision = ai_outputs.get('lab7_vision_event', {})
    motion = ai_outputs.get('lab6_motion_event', {})
    final_decision = llm.get('final_decision', {})

    return {
        'KICH_BAN': result['scenario_id'],
        'CAMERA_YOLO': {
            'trang_thai': 'Đã tích hợp webcam + YOLO' if vision.get('source') == 'webcam' else 'Chưa dùng webcam',
            'detector': vision.get('detector', 'none'),
            'so_nguoi_dem_duoc': vision.get('person_count'),
            'do_tin_cay': vision.get('confidence'),
            'bbox': vision.get('detections', []),
            'motion_detected': motion.get('motion_detected'),
            'activity_level': motion.get('activity_level')
        },
        'SO_SANH_3_TANG': {
            'sensor_only': {
                'risk_level': sensor_only.get('risk_level'),
                'ly_do': sensor_only.get('reason')
            },
            'sensor_plus_ai_models': {
                'risk_level': ai_models.get('risk_level'),
                'ly_do': ai_models.get('reason')
            },
            'sensor_ai_llm': {
                'provider': llm.get('provider'),
                'risk_level': final_decision.get('risk_level'),
                'tom_tat': final_decision.get('situation_summary'),
                'hanh_dong_de_xuat': final_decision.get('recommended_action')
            }
        },
        'SAFETY_GATE': {
            'cho_phep_dieu_khien': final_decision.get('control_allowed'),
            'can_nguoi_xem_xet': final_decision.get('need_human_review'),
            'ly_do_chan': final_decision.get('blocked_reason')
        },
        'TELEMETRY': {
            'co2_ppm': telemetry.get('co2_ppm'),
...
def gemini_chat(req: GeminiChatRequest):
    if genai is None:
        raise HTTPException(
            status_code=500,
            detail="Chưa cài google-genai. Hãy chạy: pip install google-genai"
        )
    ...
    if not api_key:
        raise HTTPException(
            status_code=500,
            detail="Thiếu GEMINI_API_KEY trong file .env"
        )
    ...
    prompt = (
        "Bạn là trợ lý AI hỗ trợ sinh viên làm Lab AIoT. "
        "Hãy trả lời bằng tiếng Việt, dễ hiểu, ngắn gọn.\n\n"
